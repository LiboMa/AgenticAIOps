...
Adapted from agenticops-chat. Provides reachability, impact analysis,
traffic path finding, anomaly detection, and network segmentation.
"""
...
def can_reach_internet(graph: InfraGraph, subnet_id: str) -> ReachabilityResult:
    """Check if a subnet can reach the Internet via IGW.

    Traces: Subnet → RouteTable → IGW (public)
...
def impact_analysis(graph: InfraGraph, failed_node_id: str) -> ImpactResult:
    """Simulate node failure and assess blast radius.
    ...
    if failed_node_id not in g:
        return ImpactResult(failed_node_id=failed_node_id, severity="unknown")
    ...
    # Create copy and remove the failed node
    g_copy = g.copy()
    g_copy.remove_node(failed_node_id)
    undirected_after = g_copy.to_undirected()

    # Nodes directly connected to the failed node
    ...
    # Subnets that lost internet connectivity
...
def detect_anomalies(graph: InfraGraph) -> AnomalyReport:
    ...
    - Blackhole routes
    - Routing cycles
    - Unreachable public subnets
    - Nodes in error status
    """
    g = graph.graph
    ...
    # Also check route table raw data
    for node_id, data in g.nodes(data=True):
    ...
    for node_id, data in g.nodes(data=True):
        if data.get("status") == NodeStatus.ERROR:
            node_type = data.get("node_type", "")
            if node_type not in (NodeType.SECURITY_GROUP,):
                anomalies.append(AnomalyItem(
                    type="unhealthy_node",
                    severity="medium",
                    node_id=node_id,
