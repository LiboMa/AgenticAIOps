...
Adapted from agenticops-chat/src/agenticops/kb/vector_store.py.
Interface unchanged — swappable to OpenSearch in Phase 2.
"""
...
@dataclass
class VectorRecord:
    """A single vector embedding record."""
    case_id: str
...
class SQLiteVectorStore(VectorStore):
    ...
    def search(
        ...
        try:
            sql = (
                "SELECT case_id, field_name, vector, resource_type, metadata_json "
                "FROM case_vectors WHERE field_name = ?"
            )
